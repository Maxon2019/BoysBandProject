...
//achtung, this library is not out of box fit for our version of qt, I will try to fix stuff here, but no promises
...
public:
    ...
    //qreal is essentially double, so all we need to do is somehow convert all floats to that
    ...
    {
        qreal backgroundValue = widget->palette().window().color().valueF(); //code tampered with, possible error
